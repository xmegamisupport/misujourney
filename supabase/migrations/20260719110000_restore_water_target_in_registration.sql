-- Regression fix: complete_registration_goals stopped populating
-- customer_goals.water_target_ml.
--
-- 20260713060000 added the NOT NULL water_target_ml column and updated this
-- RPC to compute + insert it. But 20260714180000 (nutrition targets) and
-- 20260714210000 (lowercase referral) each re-created the whole function
-- from a pre-water-target baseline, silently reverting that fix while
-- keeping the column NOT NULL. Net effect: every new customer registration
-- has been failing at the final onboarding step with
--   null value in column "water_target_ml" ... violates not-null constraint
-- ever since. This restores the water-target computation and insert on top
-- of the current (nutrition-target + lowercase-referral) function body,
...
begin
  ...
  if p_referral_code is not null and length(trim(p_referral_code)) > 0 then
    select id into v_coach_id from public.profiles where lower(referral_code) = lower(trim(p_referral_code)) and role = 'coach';
  end if;
