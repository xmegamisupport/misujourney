-- Referral codes are now generated/stored lowercase everywhere (both coach
-- creation routes); make the customer-registration lookup case-insensitive
-- too, so it doesn't matter what case a customer happens to type.
...
-- Normalize the one real coach account created so far to the new
-- lowercase convention.
update public.profiles set referral_code = lower(referral_code) where role = 'coach' and referral_code is not null;
