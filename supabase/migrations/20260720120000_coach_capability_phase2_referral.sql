-- Coach Capability — Phase 2: referral lookup + customer binding recognise
-- is_coach (with transitional role='coach' backward compatibility).
--
-- Only the coach-match condition changes in each function. complete_
-- registration_goals is recreated from its CURRENT LIVE body (fetched via
-- pg_get_functiondef) with everything else byte-identical: water target,
-- nutrition target, goal plan, goal assessment, referral normalisation,
-- onboarding completion, coach binding via coalesce.
...
as $$
  ...
  where (p.is_coach = true or p.role = 'coach')   -- Phase 2: is_coach + transitional legacy role
...
-- ---------- Customer onboarding: coach binding lookup ----------
create or replace function public.complete_registration_goals(p_customer_id uuid, p_name text, p_height numeric, p_current_weight numeric, p_age integer, p_gender text, p_phone text, p_diet_type diet_type, p_activity_level activity_level, p_goal_type goal_type, p_journey_days integer, p_long_term_goal_weight numeric DEFAULT NULL::numeric, p_referral_code text DEFAULT NULL::text, p_use_custom_goal boolean DEFAULT false, p_custom_loss_kg numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if p_referral_code is not null and length(trim(p_referral_code)) > 0 then
    -- Phase 2: coach identity is is_coach; keep role='coach' transitionally.
    select id into v_coach_id from public.profiles where lower(referral_code) = lower(trim(p_referral_code)) and (is_coach = true or role = 'coach');
  end if;
